...
import { WhereFilterOp, Query, DocumentReference } from '@firebase/firestore-types';
...
class FirestoreServiceClass {
  ...
  observeRecord(collection: string, uid: string): Observable<any> {
    return FirebaseService.firestore.collection(collection).doc(uid).valueChanges();
  }
  ...
  observeCollection(collection: string): Observable<any> {
    return FirebaseService.firestore.collection(collection).valueChanges();
  }

  getCollection(collection: string): Promise<any> {
    return FirebaseService.firestore.collection(collection).valueChanges().pipe(take(1)).toPromise();
  }
